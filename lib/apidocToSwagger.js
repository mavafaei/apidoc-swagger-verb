...
function createPostPushPutOutput (verbs, definitions, pathKeys) {
	...
	params.push({
		...
		required: required,
		schema: {
			// "$ref": "#/definitions/" + verbs.group + "/" + verbDefinitionResult.topLevelParametersRef
...
function createVerbDefinitions (verbs, definitions) {
	...
	var defaultObjectName = `${verbs.group}_${verbs.name}`

	var fieldArrayResult = {}

	if (verbs && verbs.parameter && verbs.parameter.fields) {
		fieldArrayResult = createFieldArrayDefinitions(verbs.parameter.fields.Parameter, definitions, verbs.name, defaultObjectName, verbs.group)

		result.topLevelParametersRef = fieldArrayResult.topLevelRef
	};

	if (verbs && verbs.success && verbs.success.fields) {
		fieldArrayResult = createFieldArrayDefinitions(verbs.success.fields['Success 200'], definitions, verbs.name, defaultObjectName, verbs.group)
